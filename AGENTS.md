...
# 🧠 База знаний кода (codebase-memory)
Для исследования проекта, поиска функций, классов, связей и построения архитектурных трассировок **ВСЕГДА** используйте MCP-сервер `codebase-memory` и его граф знаний.

## Приоритетные инструменты:
1. `search_graph` — поиск символов по паттерну (функции, классы, роуты, переменные)
2. `trace_path` — отслеживание вызовов (inbound/outbound)
3. `get_code_snippet` — чтение исходного кода конкретных сущностей
4. `query_graph` — сложные Cypher-запросы к графу
5. `get_architecture` — высокоуровневый обзор архитектуры

## Правила использования:
- Прежде чем использовать текстовый поиск (`grep_search` или `find`) для кода, попробуйте найти символы через `search_graph` или `trace_path`.
- **Внимание:** Внутри песочницы (sandbox) Antigravity IDE прямой вызов MCP-инструментов через встроенный плагин может выдавать ошибку `EOF` (из-за зависания бинарника на этапе инициализации памяти `mem.init`). Если это произошло, делайте прямые запросы к SQLite-базе данных `/Users/dev/.cache/codebase-memory-mcp/Users-dev-projects-pharmnode.db` с помощью `run_command` и CLI `sqlite3`!
- Для не-кодовых файлов (документация `.md`, конфигурации, Docker-файлы) или текстовых литералов / строк ошибок используйте стандартный `grep_search`.
<!-- codebase-memory-mcp:end -->

---

<!-- sequential-thinking-mcp:start -->
# 💭 Пошаговое рассуждение (sequential-thinking)
Для решения сложных логических задач, проектирования архитектуры, отладки запутанных багов и планирования многоэтапных изменений **ВСЕГДА** используйте MCP-инструмент `sequentialthinking` (`sequential-thinking`).

## Правила использования:
- Инструмент позволяет структурировать рассуждения в виде цепочки мыслей (Chain of Thought).
- При вызове инструмента обязательно передавайте все обязательные параметры, включая `thought`, `thoughtNumber`, `totalThoughts`, а также **`nextThoughtNeeded`** (установите в `false`, когда решение найдено, или `true`, если требуется продолжить цепочку мыслей).
- Корректируйте оценку общего количества шагов (`totalThoughts`) и делайте ревизии предыдущих шагов при изменении гипотез.
<!-- sequential-thinking-mcp:end -->
...
6. **Покрытие тестами**: Все новые фичи должны быть обязательно покрыты автоматическими тестами. Особое внимание следует уделять авторизации (логин/регистрация) и оплате (биллинг/подписки/вебхуки).

7. **Запрет на мануальное тестирование AI**: AI-агентам запрещено проводить мануальное (ручное) тестирование. Вместо этого AI должен детально описывать и заносить в задачи инструкции для мануального тестирования пользователем, чтобы пользователь тестировал самостоятельно.

8. **Создание скилов перед разработкой**: Если планируется разработка новой фичи, для которой в проекте еще нет специализированного скила (в каталоге `docs/skills/`), необходимо сначала создать и подробно описать этот скил (файл `SKILL.md`), и только после этого приступать к реализации самой фичи.
